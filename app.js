document.addEventListener('DOMContentLoaded', function () {
  ...
  instructions.textContent =
    'Используйте стрелки для движения и пробел для прыжка';
  ...
  const cat = {
    ...
    draw() {
      ...
      ctx.arc(
        this.x -
          cameraOffset +
          (this.direction === 'right' ? this.width / 3 : -this.width / 3),
      ...
      ctx.arc(
        this.x -
          cameraOffset +
          (this.direction === 'right' ? this.width / 2 : -this.width / 2 + 10),
        this.y - this.height / 3,
        5,
        0,
        Math.PI * 2
      ...
      ctx.moveTo(
        this.x -
          cameraOffset -
          (this.direction === 'right' ? this.width / 2 : -this.width / 2),
        this.y
      );
      ctx.quadraticCurveTo(
        this.x -
          cameraOffset -
          (this.direction === 'right' ? this.width : -this.width),
        this.y - 20,
        this.x -
          cameraOffset -
          (this.direction === 'right' ? this.width + 20 : -this.width + 20),
        this.y + 10
      );
      ctx.lineWidth = 5;
  ...
  function createPlatforms() {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const y = Math.max(
        200,
        Math.min(450, prevPlatform.y + (Math.random() > 0.5 ? -50 : 50))
      );
  ...
  function drawPlatforms() {
    platforms.forEach((platform) => {
      if (
        platform.x + platform.width > cameraOffset &&
        platform.x < cameraOffset + canvas.width
      ) {
  ...
  function drawCoins() {
    coins.forEach((coin) => {
      if (
        !coin.collected &&
        coin.x > cameraOffset - 20 &&
        coin.x < cameraOffset + canvas.width + 20
      ) {
    ...
    coins.forEach((coin) => {
      if (!coin.collected) {
        const distance = Math.sqrt(
          Math.pow(cat.x - coin.x, 2) + Math.pow(cat.y - coin.y, 2)
        );
